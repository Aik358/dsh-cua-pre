 ...
 *   1. sidebar.footer.action — 「桌面」入口按钮(开关左下角面板)
 *   2. shell.overlay         — 电脑控制面板: 实况(操作流) / 画面(帧+vision描述) / 说明
 *   3. settings.section      — 「电脑控制」设置页(基础 / 视觉识图 / 安全)
 * Data flows over /api/dsh-cua-pre/* (loopback-only host routes).
 */
console.log('[dsh-cua-pre] client v0.3.1 loading')
  ...
  factory: (require) => {
    ...
    function SidebarButton() {
      ...
      return h('button', {
        ...
        style: {
          ...
          color: 'inherit', border: 'none', borderRadius: 8, padding: '4px 8px', fontSize: 12,
        },
      }, '🖥️', h('span', null, '桌面'))
    }
    ...
    function SettingsPage() {
      ...
      return h('div', { style: { maxWidth: 640, fontSize: 13 } },
        ...
        h(Row, { label: 'Python 解释器', hint: '推荐指向项目 venv' }, txt('pythonExecutable', 'python')),
        ...
        h(Row, { label: '识图模型', hint: '留空=跟随系统路由；如 deepseek-vl 类' }, txt('visionModel', '')),
    ...
    function apply(ctx) {
      ...
      try {
        slots.inject('sidebar.footer.action', function () {
          return slots.register({ name: 'sidebar.footer.action', id: 'cua-host-pre', order: 6, label: '桌面' }, function () {
            return h(SidebarButton, null)
          })
        })
        slots.inject('shell.overlay', function () {
          // 常驻挂载，CuaPanel 内部按 useUiOpen 自隐（slots 渲染回调非组件上下文，不能在此用 hooks）
          return slots.register({ name: 'shell.overlay', id: 'cua-host-pre', order: 6 }, function () {
            return h(CuaPanel, { onClose: function () { setPanelOpen(false) } })
          })
        })
        ...
        console.log('[dsh-cua-pre] client ready: sidebar entry + panel + settings page')
      } catch (e) {
        console.warn('[dsh-cua-pre] slot registration failed', e)
      }
    }

    // overlay 渲染回调非组件上下文：面板常驻挂载、内部 useUiOpen 自隐。
