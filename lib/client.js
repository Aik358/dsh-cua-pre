 ...
 */
console.log('[dsh-cua-pre] client v0.3.4 loading')
    ...
    function apply(ctx) {
      ...
      try {
        ...
        function setPanelVisible(show) {
          ...
          if (show) {
            try { if (panelRoot._loadLive) panelRoot._loadLive() } catch (_) {}
          }
        }
        ensurePanelRoot()
        uiListeners.add(function () { setPanelVisible(uiOpen) })
        ...
        uiListeners.add(function () { renderFAB(false) })
        if (document.readyState !== 'complete') {
          document.addEventListener('DOMContentLoaded', function () { ensureFAB() })
        }
