 ...
 */
console.log('[dsh-cua-pre] client v0.5.0 loading')
  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        py_switched: 'Python 已切换并即时生效', installing_deps: '正在为该 Python 安装 uiautomation+pillow（可能 1-2 分钟）…',
      ...
      en: {
        ...
        py_switched: 'Python switched — instant effect', installing_deps: 'Installing uiautomation+pillow for this Python (may take 1-2 min)…',
    ...
    function langBtn() {
      return h('button', {
        ...
        title: '中文 / English',
        style: { background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.16)', borderRadius: 7, padding: '2px 8px', color: 'inherit', fontSize: 11, cursor: 'pointer', flexShrink: 0 },
      }, lang === 'zh' ? 'EN' : '中')
    }
          ...
          return h('div', { key: f.frame_id, style: { background: 'rgba(255,255,255,0.05)', borderRadius: 10, padding: 8 } },
            ...
            f.vision_description
              ? h('details', { style: { marginTop: 6, fontSize: 12 } },
                  h('summary', { style: { cursor: 'pointer', color: '#79c0ff' } }, t('vision_d') + ' (' + (f.vision_tiles || '?') + t('tiles')),
    ...
    function SettingsPage() {
      ...
      return h('div', { style: { maxWidth: 640, fontSize: 13 } },
          ...
          ? h('div', {
              ...
              h('button', {
                ...
                style: {
                  ...
                  borderRadius: 9, padding: '7px 16px', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer',
                },
        ...
        h('div', { style: { fontWeight: 600, margin: '14px 0 2px', color: '#ff9a8b' } }, t('sec_security')),
    ...
    function apply(ctx) {
        ...
        if (bs && typeof bs.registerTab === 'function') {
          ...
          ctx.effect(function () {
            return bs.registerTab({
              ...
              icon: function (size) { return h('span', { style: { font: '700 ' + Math.max(9, (size || 14) - 3) + 'px ui-monospace,monospace', color: '#7fb0ff' } }, 'CUA') },
              order: 40,
              single: true,
      ...
      setTimeout(whenBodyReady.bind(null, mountDom), 800) // 双保险
        ...
        function ensurePanelRoot() {
          ...
          langBtnEl.style.cssText = 'margin-left:6px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.16);border-radius:7px;padding:1px 7px;color:inherit;font-size:11px;cursor:pointer;'
          ...
          function loadLive() {
            ...
            body.innerHTML = ''
            var loading = document.createElement('div'); loading.textContent = t('loading'); loading.style.cssText = 'color:rgba(232,234,240,.45);padding:6px 0;'
            ...
            apiGet(API.state).then(function (s) {
              ...
              var ops = (s && s.lastOps) || []
              if (!ops.length) { var empty=document.createElement('div'); empty.textContent=t('no_ops'); empty.style.cssText='font-size:12px;color:rgba(232,234,240,.45);'; body.appendChild(empty); return }
          ...
          function loadFrames() {
            ...
            loading.style.cssText = 'color:rgba(232,234,240,.45);padding:6px 0;'
            body.appendChild(loading)
            function render(s) {
              ...
              if (!frames.length) {
                ...
                empty.style.cssText = 'font-size:12px;color:rgba(232,234,240,.45);line-height:1.7;padding:4px 0;'
              ...
              frames.forEach(function (f) {
                ...
                card.style.cssText = 'background:rgba(255,255,255,0.06);border-radius:10px;padding:8px;margin-bottom:10px;'
                var meta = document.createElement('div')
                meta.style.cssText = 'display:flex;gap:8px;align-items:center;font-size:11px;color:rgba(232,234,240,.55);margin-bottom:6px;flex-wrap:wrap;'
                ...
                img.onerror = function () {
                  ...
                  miss.style.cssText = 'font-size:11px;color:rgba(232,234,240,.4);padding:12px 0;text-align:center;'
                ...
                if (f.vision_description) {
                  ...
                  var sum = document.createElement('summary'); sum.textContent = t('vision_d'); sum.style.cssText = 'cursor:pointer;color:#79c0ff;'
                  var pre = document.createElement('pre'); pre.textContent = f.vision_description
                  pre.style.cssText = 'white-space:pre-wrap;margin:6px 0 0;font-size:11px;color:#c9d1d9;'
                  ...
                  ve.style.cssText = 'margin-top:6px;font-size:11px;color:rgba(232,234,240,.45);'
                  card.appendChild(ve)
                }
    ...
    function CuaPanelInline(props) {
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
        h('div', { style: { display: 'flex', gap: 6, padding: '8px 10px 0', borderBottom: '1px solid rgba(255,255,255,0.08)' } },
          ...
          h('button', { onClick: function () { setLang(lang === 'zh' ? 'en' : 'zh') }, style: { marginLeft: 'auto', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.16)', borderRadius: 7, padding: '2px 8px', color: 'inherit', fontSize: 11, cursor: 'pointer', flexShrink: 0 } }, lang === 'zh' ? 'EN' : '中'),
        ),
