 ...
 */
console.log('[dsh-cua-pre] client v0.4.7 loading')
    ...
    function CuaPanel(props) {
      ...
      return h('div', { style: Object.assign({}, GLASS, { width: props.width || 430, height: props.height || 560, display: 'flex', flexDirection: 'column', overflow: 'hidden' }) },
        h('div', { style: { display: 'flex', alignItems: 'center', padding: '10px 12px', borderBottom: '1px solid rgba(255,255,255,0.08)' } },
          ...
          h('span', { style: { marginLeft: 8, fontSize: 11, color: MUTED } }, '桌面自动化'),
          h('span', { style: { flex: 1 } }),
          h('button', {
          ...
          tab === 'live' ? h(LiveTab, { state: state })
            : tab === 'frames' ? h(FramesTab, null)
              : h('div', { style: { padding: 12, fontSize: 12, lineHeight: 1.7, color: MUTED } },
                  h('div', null, '桌面自动化控制：无障碍树优先、元素/坐标双目标、strategy 路由、写后作废、action_sent 幂等纪律。'),
    ...
    function SettingsPage() {
      ...
      function bootstrapOneClick() {
        ...
        apiPost(API.bootstrap, {}).then(function (r) {
          if (r && r.ok) {
            setBootstrapMsg('已启用 ✓（python=' + r.python + (r.depsInstalled ? '，依赖已装' : '，依赖齐全') + '）')
            set('enabled', true)
            set('pythonExecutable', r.python)
            runDetect()
          } else {
            setBootstrapMsg('失败: ' + ((r && r.error) || ''))
          }
        }).catch(function (e) { setBootstrapMsg('失败: ' + (e.message || e)) }).finally(function () { setBootstrapping(false) })
      }
      ...
      return h('div', { style: { maxWidth: 640, fontSize: 13 } },
        ...
        h('div', { style: { fontSize: 12, color: MUTED, marginBottom: 12 } }, '桌面自动化插件：无障碍树优先、30 个标准电脑控制工具。默认关闭；配置文件 ' + '~/.dsh/cua-pre.json'),
        ...
        !cfg.enabled
          ? h('div', {
              ...
              h('div', { style: { fontWeight: 700, fontSize: 13, marginBottom: 4 } }, '🚀 首次使用引导'),
              h('div', { style: { fontSize: 12, lineHeight: 1.7, color: MUTED, marginBottom: 8 } },
                '安装完成，还差一步就能用了：点击下方按钮，插件会自动检测本机 Python、缺少依赖时自动安装，然后即时启用。'),
              h('button', {
        ...
        h(Row, { label: '工具名前缀', hint: '与宿主撞名时使用；默认空=标准名' }, txt('toolNamePrefix', '')),
    ...
    function apply(ctx) {
      ...
      setTimeout(whenBodyReady.bind(null, mountDom), 800) // 双保险
        ...
        function ensurePanelRoot() {
          ...
          var badge = document.createElement('span'); badge.textContent = '桌面自动化'
          ...
          function loadFrames() {
            ...
            function render(s) {
              ...
              if (!frames.length) {
                ...
                empty.textContent = '暂无画面帧。模型执行 screenshot 或 get_app_state(include_screenshot) 后出现。'
              ...
              frames.forEach(function (f) {
                ...
                st.textContent = f.expired ? '过期' : '有效'
                ...
                if (f.vision_description) {
                  ...
                  var sum = document.createElement('summary'); sum.textContent = '[vision] 识图描述'; sum.style.cssText = 'cursor:pointer;color:#79c0ff;'
            ...
            apiGet(API.frames).then(render).catch(function (e) {
              ...
              err.textContent = '读取失败: ' + (e.message || e)
    ...
    function CuaPanelInline(props) {
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
        h('div', { style: { display: 'flex', gap: 6, padding: '8px 10px 0', borderBottom: '1px solid rgba(255,255,255,0.08)' } },
          h('button', { onClick: function () { setTab('live') }, style: tabBtnStyle(tab === 'live') }, '实况'),
          h('button', { onClick: function () { setTab('frames') }, style: tabBtnStyle(tab === 'frames') }, '画面'),
          h('button', { onClick: function () { setTab('about') }, style: tabBtnStyle(tab === 'about') }, '说明'),
        ),
