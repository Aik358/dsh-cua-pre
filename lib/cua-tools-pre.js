...
export function createCuaToolsPre(deps) {
  const { sidecar, session, gate, killSwitch, afterScreenshot, attachFrame } = deps
    ...
    async (args) => {
      ...
      if (args.target && args.target.type === 'element') {
        ...
        const r = await call('element_action', {
          sel: st.sel, path: el.p, action: 'setvalue', value: text, expectT: el.k, expectN: el.n || null,
          expectRid: el.rid || null, expectRect: el.r || null,
        }, {})
        markDispatched(st.sel.pid, r)
        if (!r.ok) throw new ToolError(r.code === 'worker-error' ? r.reason : r.code, workerMsg(r), { ambiguous: r.code === 'timeout' })
  ...
  def('set_value', '语义设置控件值',
    '直接通过 UIA ValuePattern 设置控件值（后台安全、不动真实键盘）。控件不支持时回退聚焦+注入。element target only。',
    ...
    async (args) => {
      ...
      checkPid(st.sel.pid)
        markDispatched(st.sel.pid, null)
      const r = await call('element_action', {
        sel: st.sel, path: el.p, action: 'setvalue', value: String(args.value ?? ''),
        expectT: el.k, expectN: el.n || null,
          expectRid: el.rid || null, expectRect: el.r || null,
      }, {})
      markDispatched(st.sel.pid, r)
  ...
  def('perform_action', '执行元素的命名无障碍动作',
    ...
    async (args) => {
      ...
      checkPid(st.sel.pid)
        markDispatched(st.sel.pid, null)
      const r = await call('element_action', {
        sel: st.sel, path: el.p, action: args.action, value: args.value,
        expectT: el.k, expectN: el.n || null,
          expectRid: el.rid || null, expectRect: el.r || null,
      }, {})
      markDispatched(st.sel.pid, r)
