...
export function apply(ctx) {
  ...
  // 沙箱浏览器：待打开 URL 队列（client 轮询消费）+ 最近操作窗口（实时预览）
  const pendingUrls = []
  let lastOperated = null
  ...
  /**
   * 工具声明的裸参数表 → 标准 JSON Schema（type: object + properties）。
   * 参数名直接落在 schema 节点层时，会与 JSON Schema 关键字（title/type/…）
   * 冲突并被上游网关按元校验拒绝；包进 properties 后属性名任意合法。
   * 已声明为完整 schema 节点（含 type/properties/oneOf）的保留原样。
   */
  function toToolParameters(input) {
    if (input == null) return { type: 'object', properties: {} }
    const keys = Object.keys(input)
    const isSchemaNode = keys.includes('type') || keys.includes('properties') || keys.includes('oneOf')
    if (isSchemaNode) return input
  ...
  function registerWithFallback() {
    ...
    for (const t of tools) {
      const schema = toToolParameters(t.inputSchema)
      const def = {
        ...
        output: {
          ...
          // 只挂在会产生 att= 的工具上——宿主要求 presentationMeta 返回 lossless JSON，
          // 无标记时返回 undefined 会让所有工具输出被判 invalid。
          ...(t.name === 'screenshot' || t.name === 'get_app_state' ? {
            presentationMeta: (_args, value) => {
              const m = String(value).match(/att=(sha256:[0-9a-f]+)\|b=(\d+)\|w=(\d+)\|h=(\d+)/)
              // null 是合法 JsonValue 兜底（如截图失败无 att= 时）；undefined 会被宿主判 non-lossless
              if (!m) return null
  ...
  const API = {
    ...
    'url-consumed': '/api/dsh-cua-pre/url-consumed',
  ...
  const routes = [
    {
      kind: 'exact', path: API.state, handler: async (req, res) => {
        ...
        writeJson(res, 200, {
          ...
          pendingUrl: pendingUrls.length ? pendingUrls[pendingUrls.length - 1] : null,
    ...
    {
      kind: 'exact', path: API['url-consumed'], handler: async (req, res) => {
        if (!isLoopbackRequest(req)) return writeJson(res, 403, { error: 'loopback-only' })
        if (req.method !== 'POST') return writeJson(res, 405, { error: 'method not allowed' })
        const body = await readJsonBody(req).catch(() => ({}))
        const idx = pendingUrls.findIndex((u) => u.url === body.url)
        if (idx !== -1) pendingUrls.splice(idx, 1)
        writeJson(res, 200, { ok: true })
      },
    },
    {
      kind: 'exact', path: API.watch, handler: async (req, res) => {
        if (!isLoopbackRequest(req)) return writeJson(res, 403, { error: 'loopback-only' })
        const sel = lastOperated && lastOperated.hwnd ? { hwnd: lastOperated.hwnd } : {}
        try {
          const r = await sidecar.request('screenshot', { mode: sel.hwnd ? 'window' : 'screen', sel, saveDir: config._artifactsDir, quality: 70 }, { timeoutMs: 12000 })
          if (!r.ok) return writeJson(res, 200, { ok: false, error: r.reasonText || r.code })
          writeJson(res, 200, { ok: true, path: r.payload.path, width: r.payload.width, height: r.payload.height, ts: Date.now(), title: (lastOperated && lastOperated.title) || '' })
